...
impl PipelineOrchestrator {
    pub fn new(settings: Settings, event_bus: EventBus) -> Self {
        ...
        let crawler_script = ws.join("helpers").join("playwright_crawler.js");
    ...
    pub async fn run_redesign_pipeline(
        ...
        let audit_tool = BrowserTool::new(
            self.workspace_dir.join("helpers").join("playwright_crawler.js"),
        ...
        let research_brief = research_agent.conduct_research(target_url, "Industrial Services & Auto Repair").await?;
        ...
        let design_spec = art_director.create_design_system(&research_brief, &audit_json, business_goal).await?;
        ...
        let html_content = coder_agent.generate_frontend(&design_spec, &audit_json).await?;
        ...
        if let Ok(store) = MemoryStore::open(&memory_path) {
            let summary = ProjectSummary {
                ...
                macrostructure: "Bento/Marquee Modern".to_string(),
